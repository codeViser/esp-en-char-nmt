...
def pad_sents_char(sents, char_pad_token):
    ...
    pad_word = max_word_length*[char_pad_token]
    sents_padded = [sent + ([pad_word] * (max_sent_len-len(sent))) for sent in sents]
    sents_padded = [[word[:max_word_length-1] + [word[-1]] if len(word) > max_word_length else word + ([char_pad_token] * (max_word_length-len(word))) for word in sent]for sent in sents_padded]
    ### END YOUR CODE
